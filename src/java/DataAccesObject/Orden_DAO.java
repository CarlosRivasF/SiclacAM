...
public class Orden_DAO {
    ...
    public int registrarOrden(Orden_DTO orden) {
        System.out.println("CLASE DAO (registrarOrden)");
        int r = 0;
        try (Connection con = Conexion.getCon();) {
            ...
            try (PreparedStatement pstm = con.prepareStatement(sql);) {
                ...
                sql = "select id_Orden from orden where id_unidad=" + orden.getUnidad().getId_Unidad() + " and id_paciente=" + orden.getPaciente().getId_Paciente() + ""
                        ...
                        + " and Fecha_Orden='" + orden.getFecha() + "'  and Hora_Orden='" + orden.getHora() + "' and Precio_Total='" + (orden.getTotal() + orden.getRestante()) + "' and montoRes='" + orden.getTotal() + "' and convenio='" + orden.getConvenio() + "'";
                System.out.println(sql);
