...
public class Orden_DAO {
    ...
    public int registrarOrden(Orden_DTO orden) {
        try (Connection con = Conexion.getCon();) {
            ...
            if (orden.getId_Orden() != 0) {
                ...
                if (orden.getMedico() != null & orden.getMedico().getParticipacion() != null) {
                    Float monto = orden.getMontoPagado() + orden.getMontoRestante();
                    Float montoPart = monto * (orden.getMedico().getParticipacion() / 100);
                    ...
                    participacion.setMonto(montoPart);
                    Participacion_DAO Pa = new Participacion_DAO();
                    Pa.registrarPago(participacion);
                }
            }
    ...
    public List<Orden_DTO> getOrdenes() {
        ...
        Persona_DAO Pr = new Persona_DAO();
        try (Connection con = Conexion.getCon()) {
            String sql = "SELECT * FROM orden order by id_Orden asc";
            try (PreparedStatement pstm = con.prepareStatement(sql); ResultSet rs = pstm.executeQuery();) {
                while (rs.next()) {
                    Orden_DTO ord = new Orden_DTO();
                    ord.setId_Orden(rs.getInt("id_Orden"));
